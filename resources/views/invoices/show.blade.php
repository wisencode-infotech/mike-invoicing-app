    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                ...
                @can('send', $invoice)
                    <form method="post" action="{{ route('invoices.send', $invoice) }}" onsubmit="return confirm('{{ __('Mark this invoice as sent?') }}');">
                        @csrf
                        <x-secondary-button type="submit">{{ __('Mark as Sent') }}</x-secondary-button>
                    </form>
                @endcan
                    ...
                    <ul class="mt-3 space-y-3">
                        @foreach ($invoice->eventLogs as $event)
                            <li class="text-sm">
                                ...
                                <div class="text-xs text-gray-500">{{ $event->created_at->format('M j, Y g:ia') }}</div>
                            </li>
                        @endforeach
                    </ul>
                @endif
